...
func TestHeaderStorage(t *testing.T) {
	...
	log.Info("dposContext.CandidateHash", "dposContext.CandidateHash", dposContext.CandidateHash.String())
	log.Info("dposContext.EpochHash", "dposContext.EpochHash", dposContext.EpochHash.String())
	...
	if entry := ReadHeaderRLP(db, header.Hash(), header.Number.Uint64()); entry == nil {
		...
		log.Info("解析到的DposContext", "DposContext,CandidateHash:", decodedata.DposContext.CandidateHash.String(), "EpochHash", decodedata.DposContext.EpochHash.String())
	}
...
func TestBodyStorage(t *testing.T) {
	...
	//首先确保库中没有这个hash
	//构造hash的时候，临时直接使用body的内容去计算hash，作为存储用的key中的hash；key中的number，用12作为number
	// tx1 := basic.NewTransaction(big.NewInt(time.Now().Unix()), common.HexToAddress("095e7baea6a6c7c4c2dfeb977efac326af552d8"), big.NewInt(0), []byte("test"))
	// tx = append(tx, tx1)
	// tx2 := types.NewTransaction(big.NewInt(0), common.HexToAddress("095e7baea6a6c7c4c2dfeb977efac326af552d8"), big.NewInt(0), []byte("test"))

	// body := &basic.Body{Transactions: []*basic.Transaction{tx1}}
	// body.Transactions = basic.Mocktxs()
	body := &basic.Body{Transactions: basic.Mocktxs()}
	// body.Transactions = basic.Mocktxs()

	len := len(body.Transactions)
	for i := 0; i < len; i++ {
		log.Info("存储的tx", "tx[i]", i, "body.Transactions[i]", body.Transactions[i])
	}
	//计算hash的方法
	...
	if entry := ReadBodyRLP(db, hash, number); entry == nil {
		...
		err := rlp.Decode(bytes.NewReader(entry), &decodebody)
		if err != nil {
			t.Error("bodyrlp解码失败")
		}
...
func TestBlockStorage(t *testing.T) {
	...
	block := basic.MockBlockWithtxs()

	log.Info("block的header信息", "block.Header().Hash()", block.Header().Hash().String())
	...
	log.Info("该测试block在数据库中不存在，可以开始测试存储校验")
	//写入并且校验库中的block信息
	WriteBlock(db, block)
	//校验
	if entry := ReadBlock(db, block.Header().Hash(), block.NumberU64()); entry == nil {
		...
		log.Info("db中查找到的block-entry", "entry", entry)
...
func TestHash(t *testing.T) {
	...
	log.Debug("构造的header1的hash", "hash1:", hash1(header))
	hash2 := header.Hash()
	log.Debug("构造的header2的hash", "hash2:", hash2.String())
}
