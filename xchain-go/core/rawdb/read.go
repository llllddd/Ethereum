...
func ReadBlock(db DatabaseReader, hash common.Hash, number uint64) *basic.Block {
	...
	if header == nil {
		Log.Infoln("获取block的header时，header为空")
		return nil
	}
	body := ReadBody(db, hash, number)
	if body == nil {
		Log.Infoln("获取block的body时，body为空")
		return nil
	}
	return basic.NewBlockWithHeader(header).WithBody(body)
}

// ReadHeader retrieves the block header corresponding to the hash.
// ReadHeader返回rlp(header)在decode后的Header对象
func ReadHeader(db DatabaseReader, hash common.Hash, number uint64) *basic.Header {
	...
	if err := rlp.DecodeBytes(data, &header); err != nil {
		Log.Infoln("Invalid block header RLP", "hash", hash, "err", err)
		return nil
	}
...
func ReadBody(db DatabaseReader, hash common.Hash, number uint64) *basic.Body {
	...
	if err := rlp.Decode(bytes.NewReader(data), body); err != nil {
		Log.Infoln("Invalid block body RLP", "hash", hash, "err", err)
		return nil
	}
	return body
}
